...
static inline buffer original_from_text(memory_arena *arena, u8 *text, u32 text_len)
{
    buffer buf = {};
    if (text)
    {
...
static inline buffer allocate_append_buffer(memory_arena *arena)
{
    buffer buffer;
    buffer.text_len = 0;
...
static inline base_iter find_abs_idx(base_iter *last_location, u32 abs_idx)
{
    ...
    {
        base_advance_by(&iter, abs_idx - last_abs_idx);
    }
    else
    {
...
{
    base_iter iter = find_cursor(last_location, bc);
    u32 result = get_position(&iter);
    return result;

}

static inline buffer_cursor get_cursor(base_iter *iter)
{
...
static inline base_iter find_line(base_iter *last_location, u32 line)
{
    base_iter iter = *last_location;
...
static inline base_iter find_cursor(base_iter *last_location, buffer_cursor cursor)
{
...
static inline u32 skip_space(base_iter *last_location, u32 cy)
{
    u32 result = 0;
...
static inline u32 get_line_len_(base_iter *last_location, u32 line)
{
...
static inline u32 find_char_back(base_iter *last_location, str match, u32 count, buffer_cursor cursor)
{
    u32 result = 0;
...
static iter_range range_search(
    ...
    u8 close)
{
    base_iter backward_iter = find_cursor(last_location, cursor);
    base_iter forward_iter  = backward_iter;

    // Search backwards,
    i32 back_score = 0;

    while ((back_score < 1) && base_prev_cell(&backward_iter))
    {
        str s = get_char_utf8(&backward_iter);
        Assert(s.len > 0);
    ...
    do 
    {
        str s = get_char_utf8(&forward_iter);
    ...
    } while ((forward_score < 1) && (base_next_cell_(&forward_iter)));
...
static inline buffer_range find_boundary(
    base_iter *last_location,
    u8 open,
    u8 close,
    buffer_cursor cursor)
...
static inline u32 find_char(base_iter *last_location, str match, u32 count, buffer_cursor cursor)
{
    u32 result = 0;
    base_iter iter = find_cursor(last_location, cursor);
...
static inline buffer_cursor find_word_back(base_iter *last_location, u32 count, buffer_cursor cursor)
{
...
static inline buffer_cursor find_word(base_iter *last_location, u32 count, buffer_cursor cursor)
{
    buffer_cursor result = cursor;
    base_iter iter = find_cursor(last_location, cursor);
...
static inline void copy_range(cursor start, cursor end, u32 count, slice_cursor *slice_cursor)
{
    piece *piece_start = start.node->pieces + start.piece_index;
    if (start.node == end.node)
    {
...
static inline void insert_many(piece_list *list, cursor at, piece_slice *slices, u32 num_slices)
{
...
// {
//     base_iter start = find_abs_idx(&list->iter, header->abs_idx);
//     base_iter end   = find_abs_idx(&list->iter, header->abs_idx + header->ins_count);
//
...
static inline void write_piece_text(piece_list *a, piece_range p_range, string *buf)
{
    Assert(buf->len == 0);
    ...
    for (u32 i = 0; i < p_range.count; ++i)
    {
        ...
        if ((i == 0) && (p_range.start > 0))
        {
            start = p_range.start;
        } 

        if ((i == p_range.count - 1) && (p_range.end > 0))
        {
            end = p_range.end;
        }
...
static inline piece serialize_piece_range_to(piece_list *a, piece_list *b, piece_range p_range_a)
{
    piece result = {};
    ...
    for (u32 i = 0; i < p_range_a.count; ++i)
    {
        ...
        u32 end   = a_piece->size;
        if ((i == 0) && (p_range_a.start > 0))
        {
            start = p_range_a.start;
        } 

        if ((i == p_range_a.count - 1) && (p_range_a.end > 0))
        {
            end = p_range_a.end;
        }
...
static inline replace_result yank_(piece_list *list, buffer_cursor c0, buffer_cursor c1)
{
    ...
    piece *copied_pieces = allocate_memory_block(&list->history, &list->history_arena, num_pieces * sizeof(piece));
...
static inline void replace_range(
    piece_list *list,
    base_iter start,
    base_iter end,
    piece_range p_range,
    slice_cursor s_cursor)
{
    ...
    if (p_range.start > 0 && p_range.end > 0 && p_range.count == 1) 
    {
        Assert(p_range.count == 1);
        left_piece = *(p_range.pieces);
        const buffer *buffer = get_buffer(list, left_piece.type);

        // TODO: implement search_piece to take offsets not pieces, so that off_end can 
        // start searching from off_start, also make search_piece have a flag parameter,
        // allowing the user to use linear or binary search (a lot of the times linear is desireble);
    ...
    if (start.abs_idx == end.abs_idx && start.pos_in_piece > 0) 
    {
        ...
        start_cursor = make_space(list, start_cursor, start_cursor, p_range.count + 1);
...
static inline void replace_range_2(
    piece_list *list,
    base_iter start,
    base_iter end,
    ...
    piece_slice undo_buffer)
{
    ...
    copy_range_2(start_cursor, cursor, copy_amount, undo_buffer);
    ...
    if (start.abs_idx == end.abs_idx && start.pos_in_piece > 0) 
    {
        ...
        start_cursor = make_space(list, start_cursor, start_cursor, inserted_pieces.count + 1);
...
static inline void range_replace_2(
    piece_list *list, 
    buffer_cursor c0,
    buffer_cursor c1,
    piece_slice p_slice)
{
    ...
    replace_range_2(list, start, end, p_slice, undo_buffer);
}
    ...
    piece_range p_range)
{
    ...
    replace_range(list, start, end, p_range, slice_cursor);
    result.undo_header = header;
    return result;
}

static inline void initialize_piece_list(piece_list *list, u8 *original_text, u32 original_text_len)
{
...
static inline buffer_cursor rand_buffer_cursor(piece_list *list, prng *prng)
{
...
static inline buffer_range rand_buffer_range(piece_list *list, prng *prng)
{
    // Assert(list->size);
    buffer_range result = {};
    b32 found = false;
...
static inline replace_result rand_replace(piece_list *list, prng *prng)
{
    replace_result result = {};
...
static inline b32 rand_replace_2(piece_list *list, prng *prng)
{
    if (list->size + num_insert_pieces == 0) 
    {
        return false;
    ...
    {
        range_replace_2(list, a, b, p_slice);
    }
    free(pieces);
    return true;
}
...
static inline piece_list *rand_list_2(prng *prng)
{
    piece_list *list = BootstrapPushStruct(piece_list, list_arena, 4096);
...
#if 0
static inline cursor make_space_for_at(piece_list *list, cursor at, u32 count)
{
    cursor result;
...
//
//
//             undo_once(list, header, redo_header);
//
//             redo_header->next = prev_header;
//             prev_header = redo_header;
//         }
//
//         free_undo_memory_block(&list->history_arena, &list->history, *first_header);
