...
static b32 rand_replace(piece_list *list, prng *prng)
{
    ...
    replace_range_(list, start, end, pieces, types, num_ins_pieces);
    free(pieces);
    return true;
}
...
static void insert_mode_sequence(piece_list *list, prng *prng, insert_seq_result *seq)
{
    // u32 max_length_added = MAX_INSERT_MODE_SEQ * MAX_STRING_LEN;

    // Create a big enough string to hold the largest amount of text possible.
    // INIT_STACK_STRING(text_added, max_length_added);
    ...
    for (u32 i = 0; i < seq_size; ++i)
    {
        if (rand_b32(prng))
        {
            ...
            for (u32 j = 0; j < text.len; ++j)
            {
                ...
                insert_mode_insert(list, c);
            }
        }
        else
        {
            // Note *list->pos - list->cx* : In insert mode we don't delete lines.
            // This is because the implementation is simpler and it is more 
            // efficient to delete lines in normal mode, thus encouroging the most 
            // efficient editing style.
            u32 begin = rand_range_u32_inclusive(prng, list->pos - list->cx, list->pos);
            u32 delete_amount = list->pos - begin;
            ...
            u32 curr_min = Minimum(list->pos, begin);
            seq->min = Minimum(seq->min, curr_min);
            for (; list->pos > begin;)
            {
                insert_mode_delete(list);
            }
        }
    }
...
void insert_mode_seq(prng *p)
{
    ...
    u32 max_length_added = MAX_INSERT_MODE_SEQ * MAX_STRING_LEN;
    move_to(list_a, rand_range_u32_inclusive(p, 0, list_a->size));
    insert_seq_result *seq = create(max_length_added);
    insert_mode_sequence(list_a, p, seq);
    commit_insert_mode_undo(list_a);

    piece_list *list_b = rand_list(&clone);
    {
        base_iter start = find_position(&list_b->iter, seq->min);
        base_iter end   = find_position(&list_b->iter, seq->max);
        if (seq->text_added.len > 0)
        {
            piece piece = make_piece_s(list_b, seq->text_added);
            buffer_type type = BufferType_Append;
            replace_range_(list_b, start, end, &piece, &type, 1);
        }
        else if (seq->max > seq->min)
        {
            replace_range_(list_b, start, end, 0, 0, 0);
        }
    }
